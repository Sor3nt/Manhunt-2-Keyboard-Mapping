...
// Map to store references to the keybinding UI items for easy updating.
std::map<int, CNeoItem*> g_keyBindingItemsMap;

// Array to hold the default key mappings. Size 64 for safety.
// FIX: Changed from combined declaration and initialization to separate steps
// to avoid potential compiler-specific syntax errors.
KeyMapping g_defaultMapping[64];

// Populates the default keymap with standard PC controls.
void populateDefaultKeymap() {
    g_defaultMapping[KeyboardAction::ACTION_MOVE_FORWARD].Code = Keyboard::KEY_W;
    g_defaultMapping[KeyboardAction::ACTION_MOVE_BACKWARDS].Code = Keyboard::KEY_S;
    g_defaultMapping[KeyboardAction::ACTION_MOVE_LEFT].Code = Keyboard::KEY_A;
    g_defaultMapping[KeyboardAction::ACTION_MOVE_RIGHT].Code = Keyboard::KEY_D;
    g_defaultMapping[KeyboardAction::ACTION_PEEKL].Code = Keyboard::KEY_Q;
    g_defaultMapping[KeyboardAction::ACTION_PEEKR].Code = Keyboard::KEY_E;
    g_defaultMapping[KeyboardAction::ACTION_LOOKBACK].Code = Keyboard::KEY_T;
    g_defaultMapping[KeyboardAction::ACTION_GRAPPLE].Code = Keyboard::KEY_F;
    g_defaultMapping[KeyboardAction::ACTION_RUN].Code = Keyboard::KEY_LSHIFT;
    g_defaultMapping[KeyboardAction::ACTION_CRAWL].Code = Keyboard::KEY_Z;
    g_defaultMapping[KeyboardAction::ACTION_CROUCH].Code = Keyboard::KEY_LContol;
    g_defaultMapping[KeyboardAction::ACTION_SNEAK].Code = Keyboard::KEY_C;
    g_defaultMapping[KeyboardAction::ACTION_WALLSQUASH].Code = Keyboard::KEY_SPACE;
    g_defaultMapping[KeyboardAction::ACTION_CLIMB].Code = Keyboard::KEY_SPACE;
    g_defaultMapping[KeyboardAction::ACTION_FIRE1].Code = Keyboard::KEY_MOUSE_LEFT;
    g_defaultMapping[KeyboardAction::ACTION_RELOAD].Code = Keyboard::KEY_R;
    g_defaultMapping[KeyboardAction::ACTION_PUNCH].Code = Keyboard::KEY_MOUSE_RIGHT;
    g_defaultMapping[KeyboardAction::ACTION_EXECUTE].Code = Keyboard::KEY_E;
    g_defaultMapping[KeyboardAction::ACTION_USE].Code = Keyboard::KEY_E;
    g_defaultMapping[KeyboardAction::ACTION_INVENTORY_SLOT1].Code = Keyboard::KEY_1;
    g_defaultMapping[KeyboardAction::ACTION_INVENTORY_SLOT2].Code = Keyboard::KEY_2;
    g_defaultMapping[KeyboardAction::ACTION_INVENTORY_SLOT3].Code = Keyboard::KEY_3;
    g_defaultMapping[KeyboardAction::ACTION_INVENTORY_SLOT4].Code = Keyboard::KEY_4;
    g_defaultMapping[KeyboardAction::ACTION_MENU].Code = Keyboard::KEY_TAB;
    g_defaultMapping[KeyboardAction::ACTION_PICKUP].Code = Keyboard::KEY_E;
    g_defaultMapping[KeyboardAction::ACTION_DROP].Code = Keyboard::KEY_G;
    g_defaultMapping[KeyboardAction::ACTION_BLOCK].Code = Keyboard::KEY_MOUSE_RIGHT;
    g_defaultMapping[KeyboardAction::ACTION_ZOOMIN].Code = -30; // Mouse Wheel Up
    g_defaultMapping[KeyboardAction::ACTION_ZOOMOUT].Code = -31; // Mouse Wheel Down
    // FIX: Replaced undeclared KEY_MOUSE_MIDDLE with KEY_TAB as a valid placeholder.
    g_defaultMapping[KeyboardAction::ACTION_ZOOMAIM].Code = Keyboard::KEY_TAB;
    g_defaultMapping[KeyboardAction::ACTION_LOCKON].Code = Keyboard::KEY_TAB;
    // FIX: Replaced undeclared KEY_LALT with KEY_LMENU, which is defined.
    g_defaultMapping[KeyboardAction::ACTION_LOOKAROUND].Code = Keyboard::KEY_LMENU;
}

// Resets all controls to their default values and updates the UI.
void resetControlsToDefaults() {
    // Iterate through the map of UI items we saved earlier.
    // FIX: Replaced C++17 structured binding with C++11 compatible loop.
    for (auto const& pair : g_keyBindingItemsMap)
    {
        int action = pair.first;
        CNeoItem* item = pair.second;
        if (item) {
            // Get the default key code for this action.
            int defaultKeyCode = g_defaultMapping[action].Code;

            // Update the game's active key mapping.
            CInput::activeMapping[action].Code = defaultKeyCode;

            // Update the UI label to show the new default key.
            item->setKeyBindingLabel(defaultKeyCode);
        }
    }
}
...
void addBindingEntry(LayeredPage* controllerPage, char* index, float left, float top, KeyboardAction action, char* actionStr) {
    ...
    // Store the created item in our map for easy access later.
    // The 'action' enum value is the key.
    g_keyBindingItemsMap[action] = item;
...
void CreateLayeredPage() {
    ...
    // Populate the default key mappings once.
    static bool defaults_populated = false;
    if (!defaults_populated) {
        populateDefaultKeymap();
        defaults_populated = true;
    }
    ...
    */
    ...
    controllerPage->addItemToPage(text->create("Notice: Use arrow and enter keys to set new keybinds", 0.55, 0.89, 0.7, 0.7));
    ...
    //right
    addBindingEntry(controllerPage, "grapple", 0.52, 0.25, KeyboardAction::ACTION_GRAPPLE, "12");
    ...
    // FIX: Changed internal name to 'reset_defaults' for identification.
    controllerPage->addItemToPage(linkToPage->create("reset_defaults", "Reset Defaults", 0.3, 0.90, 0));
    ...
    */
    ...
    NeoMenu::setDefaultColor(150, 30, 0, 170);
    controllerPage->addItemToPage(text->create("Coded by Sor3nt", 0.80, 0.02, 0.8, 0.8));
    controllerPage->addItemToPage(text->create("Notice: Use arrow and enter keys to set new keybinds", 0.55, 0.89, 0.7, 0.7));
    controllerPage->addItemToPage(text->create("ACTMAP_T", 0.08, 0.124, 1.3, 1.4));
    NeoMenu::setDefaultColor(200, 210, 175, 170);

    //left
    addBindingEntry(controllerPage, "fire/hard punch", 0.1, 0.25, KeyboardAction::ACTION_FIRE1, "7");
    ...
    //right
    addBindingEntry(controllerPage, "A_LKBK_PC", 0.52, 0.25, KeyboardAction::ACTION_MENU, "24");
...
// Add critical section for thread safety
CRITICAL_SECTION g_cs;
bool g_waitForInject = true;
...
void Init()
{
    ...
    // FIX: Initialize the global array here to avoid compiler issues.
    memset(g_defaultMapping, 0, sizeof(g_defaultMapping));
    ...
    while (true)
    {
        // State 1: Wait for main menu initialization
        if (g_waitForInject) {
            if (NeoMenu::isCurrentPageSetTo("start")) {
                CreateLayeredPage();

                // Safe page modification
                ...
                if (controlsPage) {
                    ...
                    if (movementItem) movementItem->setText1("mymovementkeys");
                    if (actionItem) actionItem->setText1("myactionkeys");
                }
                g_waitForInject = false;
        ...
        // Check if we're in a key binding page
        bool inBindingPage = NeoMenu::isCurrentPageSetTo("mymovementkeys") ||
            NeoMenu::isCurrentPageSetTo("myactionkeys");

        // State 2: Key binding mode
        ...
        if (localWaitToApply) {
            // Add cooldown check to prevent race conditions
            const DWORD currentTime = GetTickCount();
            if (currentTime - g_lastKeyTime > 300) {
                ...
                if (pressedKeyCode != -1) {
                    ...
                    Sleep(200); // Post-binding cooldown
                }
            }
            continue;
        }

        // State 3: Normal input handling - only process Enter in binding pages
        if (KeyHit(VK_RETURN) && inBindingPage) {
            // Cooldown check
            const DWORD currentTime = GetTickCount();
            if (currentTime - g_lastKeyTime > 300) {
                g_lastKeyTime = currentTime;
                ...
                if (item) {
                    // FIX: Check if the selected item is the reset button.
                    // The 'getName()' method does not exist on CNeoItem.
                    // This assumes CNeoItem has a public C-string member 'm_pszName' for its internal name,
                    // which is a common pattern in older C++ game engines.
                    if (item->m_pszName && strcmp(item->m_pszName, "reset_defaults") == 0) {
                        resetControlsToDefaults();
                        Sleep(200); // Prevent immediate re-trigger
                    }
                    else if (item->isKeybinding()) {
                        item->setKeyBindingLabel(-1); // Show "PRESS A KEY"

                        // Safe state update
                        EnterCriticalSection(&g_cs);
                        g_currentBindingItem = item;
                        g_waitToApply = true;
                        LeaveCriticalSection(&g_cs);
    ...
    // Cleanup (though thread runs indefinitely)
